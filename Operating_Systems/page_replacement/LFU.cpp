...
int find_LFU(int frame[], int frame_size, std::map<char, int> references) {
    ...
    */
    ...
    for(index=0;index<frame_size;index++) {
        ...
        if(ch == min_key) {
            break; //return the index of page in frame that can be replaced
        }
    }
    return index;
}
    ...
    for(int i=0;i<reference_string.length();i++) { 
        ...
        if(counter < frame_size) {
            ...
            if (is_current_page_in_frame(frame, frame_size, current_page)) {
                ...
                //find the LFU page and replace it with current page
                int index = find_LFU(frame, frame_size, references);
                std::cout << "Least frequently used: Page " << frame[index] << std::endl;
                std::cout << "-------------------------------\n";
