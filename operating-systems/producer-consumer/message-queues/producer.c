...
void sendMessage(int qid, Message *msg) {
    ...
    if (msgsnd(qid, msg, msg_size, 0) == -1) {
        ...
        printf("\n=> PRODUCER: Successfully sent a type %d message containing an item of size %d bytes, with contents: data1 = %d, data2 = %d.\n", msg->mtype, msg_size, msg->item.data1, msg->item.data2);
    }
}
